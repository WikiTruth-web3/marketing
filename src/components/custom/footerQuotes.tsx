...
const FooterQuotes: React.FC = () => {
  ...
  return (
    <div className="
    flex flex-col gap-2 max-w-md min-h-[80px]
    ">
      <p
        className={`text-white/60 text-sm md:text-base 
          line-clamp-2
          italic transition-opacity duration-500 
          ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}
      >
        "{quote.proverb}"
      </p>
      <p
        className={`
          ...
          transition-opacity duration-500 ease-in-out delay-100 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
      >
        — {quote.from || 'Unknown'}
      </p>
    </div>
