...
export const RoadmapCard: React.FC<RoadmapCardProps> = ({
  ...
  const colorClasses = {
    'primary': 'border-l-primary hover:border-primary',
    ...
    'pink': 'border-l-pink-500 hover:border-pink-500',
    'red': 'border-l-red-500 hover:border-red-500',
  };
    ...
    `}>
      ...
      `}>
        <div className={`
          ...
          `}
        >
          <span className={`${colorClasses2[color]} text-xs tracking-widest block mb-2 opacity-60`}>
            {quarter}
          </span>
          <h3 className="text-lg md:text-xl lg:text-2xl font-bold text-white mb-4">{phase}: {title}</h3>
          <p className="text-white/80 leading-relaxed mb-6
          text-xs md:text-sm lg:text-base
          ">
            {description}
          </p>
          <ul className="space-y-2 text-[11px] md:text-xs lg:text-sm text-white/50">
            {items.map((item, idx) => (
      ...
      <div className="order-1 lg:order-2 flex flex-col items-center shrink-0">
        <div className={`w-20 h-20 rounded-full flex items-center justify-center border relative ${iconBgClasses[color]}`}>
          ...
          {isActive /* Only active gets ping ring */ && (
            <div className={`absolute inset-0 rounded-full border opacity-30 animate-ping ${color === 'primary' ? 'border-primary' :
