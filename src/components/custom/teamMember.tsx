...
export const TeamMember: React.FC<TeamMemberProps> = ({ name, role, img, type, github, twitter, telegram }) => {
  ...
  return (
    <div className={`bg-surface-low hover:bg-surface p-6 rounded-xl flex flex-col gap-4 group transition-all duration-300 ${glowColor[type]} hover:-translate-y-1`}>
      ...
      <div className="space-y-1">
        ...
        <p className={`text-xs uppercase tracking-widest ${type === 'green' ? 'text-primary' : type === 'blue' ? 'text-blue-400' : type === 'cyan' ? 'text-cyan-400' : 'text-purple-400'
          }`}>
          {role}
        </p>
      </div>
